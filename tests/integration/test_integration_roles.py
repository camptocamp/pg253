...
def setup():
    ...
    os.environ['EXCLUDE_DATABASES'] = 'pguser|postgres|template.*|big|application|data'
    os.environ['AWS_S3_PREFIX'] = 'pg253-integration-tests/'
    os.environ['BACKUP_ROLES'] = 'true'
    s3 = boto3.resource(
            's3',
...
def test_backup_roles_success(_):
    ...
    with tempfile.TemporaryDirectory() as tmpdirname:
        for i, obj in enumerate(s3_bucket.objects.all()):
            ...
            assert obj.size > 100 # Arbitrary value to ensure the object is not empty

            backup_path = f"{tmpdirname}/{obj.key.replace('/', '_')}"
            s3_bucket.download_file(obj.key, backup_path)

            with open(backup_path, "r", encoding='utf-8') as backup_file:
                content = backup_file.read()
                assert re.search('^-- PostgreSQL database cluster dump$',
                                 content, re.MULTILINE)
                assert re.search('^-- PostgreSQL database cluster dump complete$',
                                 content, re.MULTILINE)
